...
MODEL_PATH = MODEL_DIR / "species_classifier_best.pth"
CLASSES_PATH = MODEL_DIR / "species_classes.json"
...
def _load_model():
    ...
    if not MODEL_PATH.exists():
        raise FileNotFoundError(f"Species classifier model not found at {MODEL_PATH}")
...
def predict(pil_image, top_k=3):
    ...
    for idx in top_indices:
        predictions.append({
            "species": class_names[idx].replace("_", " "),
